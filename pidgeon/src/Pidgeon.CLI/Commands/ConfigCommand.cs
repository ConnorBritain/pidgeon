...
public class ConfigCommand : BaseCommand
{
    ...
    {
        var samplesOption = new Option<string>("--samples", "Path to directory containing sample messages")
        {
            Required = true
        };

        var vendorOption = new Option<string>("--vendor", "Vendor name (e.g., Epic, Cerner)")
        {
            Required = true
        };

        var standardOption = new Option<string>("--standard", "Standard name (e.g., HL7v23, FHIRv4)")
        {
            Required = true
        };

        var messageTypeOption = new Option<string>("--type", "Message type (e.g., ADT^A01, Patient)")
        {
            Required = true
        };

        var outputOption = new Option<string?>("--output", "Output file for configuration (optional)");
        ...
        command.SetAction(async (parseResult, cancellationToken) =>
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"Error: Samples directory not found: {samples}");
                    return 1;
                }

                // Read all .hl7 files from the directory
                var messageFiles = Directory.GetFiles(samples, "*.hl7");
                if (messageFiles.Length == 0)
                {
                    Console.WriteLine($"No .hl7 files found in: {samples}");
                    return 1;
                }
                ...
                Console.WriteLine($"Analyzing {messages.Count} messages for {address}...");

                // Analyze messages
                var result = await _configCatalog.AnalyzeMessagesAsync(messages, address);
            ...
            {
                Logger.LogError(ex, "Error during configuration analysis");
    ...
    {
        var vendorOption = new Option<string?>("--vendor", "Filter by vendor");
        var standardOption = new Option<string?>("--standard", "Filter by standard");
        ...
        command.SetAction(async (parseResult, cancellationToken) =>
        {
            ...
            {
                Logger.LogError(ex, "Error listing configurations");
                Console.WriteLine($"Error: {ex.Message}");
                return 1;
            }
        });
    ...
    {
        var addressOption = new Option<string>("--address", "Configuration address (Vendor-Standard-MessageType)")
        {
            Required = true
        };
