...
/// Can be run as a console app if needed for quick validation.
/// </summary>
public class SimpleCsvParserTest
{
    ...
    private static async Task TestFileDiscovery(MappingImportService importService)
    {
        ...
        var segmentFiles = files.Where(f => f.FileType == Core.Domain.Interop.MappingFileType.Segment).Count();
        var datatypeFiles = files.Where(f => f.FileType == Core.Domain.Interop.MappingFileType.DataType).Count();
        var codesystemFiles = files.Where(f => f.FileType == Core.Domain.Interop.MappingFileType.CodeSystem).Count();
