...
public class GenerateCommand : CommandBuilderBase
{
            ...
            try
            {
                ...
                if (result.IsSuccess)
                {
                    if (!string.IsNullOrEmpty(output))
                    {
                        ...
                        if (count == 1)
                        {
                            Console.WriteLine($"✅ Generated {count} {request.MessageType} message and saved to {output}");
                        }
                        else
                        {
                            var directory = Path.GetDirectoryName(output) ?? ".";
                            var filePattern = $"{Path.GetFileNameWithoutExtension(output)}_*{Path.GetExtension(output)}";
                            Console.WriteLine($"✅ Generated {count} {request.MessageType} messages and saved to {Path.Combine(directory, filePattern)}");
                        }
                    }
                    else
                    {
    ...
    private async Task WriteToFileAsync(string filePath, IReadOnlyList<string> messages, string format)
    {
        ...
        // Determine padding width based on message count (e.g., 001, 002 for <1000 messages)
