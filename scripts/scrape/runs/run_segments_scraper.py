#!/usr/bin/env python3
"""
Run the HL7 Segments scraper on all segments with quality tracking
"""
...
def run_segments_scraper(limit=None, test_mode=True):
    """Run component scraper on all segments"""
    ...
    dest_path = Path(f"../outputs/segments/{run_dir}")
    dest_path.mkdir(parents=True, exist_ok=True)

    print(f"Running HL7 Segments scraper...")
    print(f"Mode: {'TEST' if test_mode else 'PRODUCTION'}")
    print(f"Output directory: {dest_path}")
    print(f"Limit: {limit if limit else 'All segments'}")
...
def analyze_results(dest_path):
    ...
    print(f"\n=== SEGMENTS SCRAPER RESULTS ANALYSIS ===")

    try:
        # Count individual files
        individual_files = list(dest_path.glob("v23_*.json"))
        print(f"Total segments processed: {len(individual_files)}")

        # Check for master file
        master_file = dest_path / "segments_v23_master.json"
        if master_file.exists():
            with open(master_file, 'r', encoding='utf-8') as f:
                master_data = json.load(f)
            print(f"Master file contains: {len(master_data)} segments")

            # Sample analysis
            if master_data:
                sample = master_data[0]
                print(f"Sample segment: {sample.get('code', 'Unknown')}")
            ...
            field_counts = [len(item.get('fields', [])) for item in master_data]
            if field_counts:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run HL7 Segments Scraper')
    parser.add_argument('--limit', type=int, help='Limit number of segments to scrape')
    parser.add_argument('--prod', action='store_true', help='Production mode (default: test mode)')

    args = parser.parse_args()

    success = run_segments_scraper(limit=args.limit, test_mode=not args.prod)
