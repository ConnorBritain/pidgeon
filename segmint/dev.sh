...
# Configurable dotnet path - modify this to point to your dotnet installation
# Common paths:
# Windows: "C:\Program Files\dotnet\dotnet.exe" or "/mnt/c/Program Files/dotnet/dotnet.exe"
# Linux: "/usr/share/dotnet/dotnet" or "/usr/bin/dotnet"
# macOS: "/usr/local/share/dotnet/dotnet"
DOTNET_PATH="${DOTNET_PATH:-dotnet}"  # Default to 'dotnet' in PATH

# Check if .NET is installed
check_dotnet() {
    if ! command -v "$DOTNET_PATH" &> /dev/null; then
        log_error ".NET 8 SDK is not found at: $DOTNET_PATH"
        log_info "Set DOTNET_PATH environment variable or modify dev.sh line 23"
        log_info "Common paths:"
        log_info "  Windows: export DOTNET_PATH=\"/mnt/c/Program Files/dotnet/dotnet.exe\""
        log_info "  Linux:   export DOTNET_PATH=\"/usr/share/dotnet/dotnet\""
        log_info "  Or install from: https://dotnet.microsoft.com/download/dotnet/8.0"
        exit 1
    fi
    
    log_info "Using .NET version: $($DOTNET_PATH --version)"
    log_info "Using .NET path: $DOTNET_PATH"
}
...
cmd_build() {
    ...
    "$DOTNET_PATH" build --configuration Debug --verbosity minimal
...
cmd_test() {
    ...
    "$DOTNET_PATH" test --logger console --verbosity normal
}

cmd_test_watch() {
    log_info "Starting test watcher..."
    "$DOTNET_PATH" watch --project tests/Segmint.Tests test
}

cmd_run_cli() {
    log_info "Running Segmint CLI..."
    "$DOTNET_PATH" run --project src/Segmint.CLI -- "$@"
}
...
cmd_status() {
    ...
    if "$DOTNET_PATH" test --nologo --verbosity quiet > /dev/null 2>&1; then
    ...
    if "$DOTNET_PATH" build --nologo --verbosity quiet > /dev/null 2>&1; then
